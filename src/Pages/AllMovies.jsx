...
const AllMovies = () => {
  ...
  return (
    <div className="container mx-auto py-8">
      <h2 className="md:text-4xl text-3xl font-bold text-center mb-6 text-fuchsia-600">All Movies</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {isLoading
          ...
          ? filteredMovies.map((movie) => (
              <div key={movie._id} className="bg-white rounded shadow-lg p-4 h-full">
                <img
                  src={movie.poster}
                  alt={movie.title}
                  className="w-full h-[200px] object-cover rounded mb-4"
                />
                <h3 className="text-xl text-fuchsia-600 font-semibold mb-2">{movie.title}</h3>
